...
const createRealAppleHealthData = async () => {
  try {
    ...
    if (!user) {
      ...
      console.log('Found users:', allUsers.map(u => ({ email: u.email, name: `${u.firstName} ${u.lastName}` })));
      return;
    }
    ...
    console.log('\n=== Apple Health Data Created (Railway) ===');
    ...
    console.log('\n✅ Real Apple Health data created successfully on Railway!');
