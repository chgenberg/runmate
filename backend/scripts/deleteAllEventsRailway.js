...
const deleteAllEventsRailway = async () => {
  try {
    // Railway MongoDB URI
    ...
    if (!RAILWAY_MONGODB_URI) {
      ...
      console.log('5. Kör: MONGODB_URI="din-uri-här" node scripts/deleteAllEventsRailway.js');
      process.exit(1);
    }
    
    console.log('🚨 DELETE ALL EVENTS FROM RAILWAY DATABASE');
    console.log('==========================================');
    ...
    console.log('⚠️  WARNING: This will delete ALL events from Railway database!');
    ...
    console.log(`✅ Successfully deleted ${result.deletedCount} events from Railway`);
    console.log('🎉 Railway database is now clean and ready for new events!');
