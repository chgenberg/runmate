...
// Modern Challenge Card Component - Mobile Optimized
          ...
          <div className="p-4 md:p-5">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl bg-gradient-to-br ${getDifficultyColor(challenge.type)} flex items-center justify-center shadow-lg flex-shrink-0`}>
                  {getIcon(challenge.type)}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-base md:text-lg font-bold text-gray-900 group-hover:text-orange-600 transition-colors truncate">
                    {challenge.title}
                  </h3>
              ...
              {isJoined && (
                <div className="px-3 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full flex-shrink-0">
                  Deltar
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div className="text-center bg-gray-50 rounded-lg p-2">
                <div className="text-lg md:text-xl font-bold text-gray-900">{participantCount}</div>
                <div className="text-xs text-gray-500">Deltagare</div>
              </div>
              <div className="text-center bg-gray-50 rounded-lg p-2">
                <div className="text-lg md:text-xl font-bold text-gray-900">{daysRemaining}</div>
                <div className="text-xs text-gray-500">Dagar kvar</div>
              </div>
              <div className="text-center bg-gray-50 rounded-lg p-2">
                <div className="text-sm md:text-base font-bold text-gray-900 leading-tight">{getGoalDisplay()}</div>
              ...
              <div className="flex -space-x-2">
                ...
                {participantCount > 4 && (
                  <div className="w-8 h-8 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center">
                    <span className="text-xs font-medium text-gray-600">+{participantCount - 4}</span>
                  </div>
                )}
              </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 pb-20 lg:pb-0">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-4 pt-6 pb-8 md:pt-8 md:pb-12">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <motion.div
            ...
            className="grid grid-cols-3 gap-3 md:gap-4 max-w-4xl mx-auto mt-6 md:mt-8"
          >
            <div className="bg-white rounded-xl p-3 md:p-6 shadow-lg border border-gray-100">
              <div className="flex flex-col items-center">
                <Trophy className="w-6 h-6 md:w-8 md:h-8 text-orange-500 mb-1 md:mb-2" />
                <span className="text-xl md:text-3xl font-bold text-gray-900">{totalChallenges}</span>
                <p className="text-gray-600 text-xs md:text-sm">Totalt</p>
              </div>
            </div>
            
            <div className="bg-white rounded-xl p-3 md:p-6 shadow-lg border border-gray-100">
              <div className="flex flex-col items-center">
                <Flame className="w-6 h-6 md:w-8 md:h-8 text-red-500 mb-1 md:mb-2" />
                <span className="text-xl md:text-3xl font-bold text-gray-900">{activeChallenges}</span>
                <p className="text-gray-600 text-xs md:text-sm">Aktiva</p>
              </div>
            </div>
            
            <div className="bg-white rounded-xl p-3 md:p-6 shadow-lg border border-gray-100">
              <div className="flex flex-col items-center">
                <Award className="w-6 h-6 md:w-8 md:h-8 text-green-500 mb-1 md:mb-2" />
                <span className="text-xl md:text-3xl font-bold text-gray-900">{completedChallenges}</span>
      ...
      <div className="px-4 pb-20">
        ...
        >
          ...
          <div className="flex items-center justify-start md:justify-center gap-2 mb-4 md:mb-6 overflow-x-auto scrollbar-hide pb-2">
            {[
              { id: 'trending', label: 'Trendande', icon: Flame },
              { id: 'my', label: 'Mina', icon: Star },
              { id: 'all', label: 'Alla', icon: Flag }
            ].map((tab) => (
              <motion.button
                key={tab.id}
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              ...
              className="w-full pl-12 pr-12 py-3 md:py-4 bg-white rounded-xl md:rounded-2xl shadow-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all text-base"
            />
            {searchTerm && (
              <button
          ...
          {/* Create Challenge Button - Fixed on mobile */}
          <div className="fixed bottom-24 right-4 md:relative md:bottom-auto md:right-auto md:text-center md:mt-6 z-30">
            <Link to="/app/challenges/create">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold px-4 py-3 md:px-6 rounded-full md:rounded-xl shadow-lg hover:shadow-xl transition-all"
              >
                <Plus className="w-5 h-5" />
                <span className="hidden md:inline">Skapa ny utmaning</span>
              </motion.button>
            </Link>
          </div>
        </motion.div>
